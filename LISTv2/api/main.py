...
from api import bias_engine, caldera_abilities
...
from config import ATTACHMENTS_DIR, BACKUP_DIR, BIAS_PATH, CALDERA_PATH, REPORTS_DIR
...
async def lifespan(app: FastAPI):
    ...
    if CALDERA_PATH:
        log.info("Loading CALDERA abilities from %s …", CALDERA_PATH)
        caldera_abilities.init_caldera(CALDERA_PATH)
    else:
        log.info("No CALDERA_PATH configured or discovered; adversarial action lookup disabled")
    yield
...
def health():
    return {
        "status": "ok",
        "bias_ready": bias_engine.is_ready(),
        "caldera_ready": caldera_abilities.is_ready(),
    }
